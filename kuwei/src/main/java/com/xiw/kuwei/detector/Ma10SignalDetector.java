...
/**
 * 10日均线交易信号识别器
 * 规则：收盘价上穿10日均线 → 买入（强度=1）
 * 收盘价下穿10日均线 → 卖出（强度=1）
 */
@Detector(name = "10日均线交易信号识别器")
public class Ma10SignalDetector implements DetectorInterface {
    ...
    /**
     * 确保数据中 MA10 已计算，若未计算则动态计算并填充
     */
    private static void ensureMa10Calculated(List<StockDailyInfoVO> data) {
        for (int i = 0; i < data.size(); i++) {
            StockDailyInfoVO vo = data.get(i);
            if (vo.getMaInfo() == null) {
                vo.setMaInfo(new com.xiw.kuwei.calculator.MaInfo());
            }
            if (vo.getMaInfo().getMa10() != null) {
                continue; // 已存在，跳过
            }
            ...
            if (i >= MA_PERIOD - 1) {
                ...
                BigDecimal ma10 = sum.divide(BigDecimal.valueOf(MA_PERIOD), 4, RoundingMode.HALF_UP);
                vo.getMaInfo().setMa10(ma10);
            }
        }
    }
     ...
     * 检测所有信号
     *
     * @param data 包含均线信息的日线数据（需已计算 MA10）
     * @param code 股票代码
     * @return 信号列表
     */
    public List<Signal> detectSignals(List<StockDailyInfoVO> data, String code) {
        ...
        // 1. 确保数据中 MA10 已计算，若未计算则动态计算
        ensureMa10Calculated(data);
        ...
        for (int i = 1; i < data.size(); i++) {
            ...
            BigDecimal prevMa = prev.getMaInfo() != null ? prev.getMaInfo().getMa10() : null;
            BigDecimal currMa = curr.getMaInfo() != null ? curr.getMaInfo().getMa10() : null;

            if (prevClose == null || currClose == null || prevMa == null || currMa == null) {
                continue;
            }

            // 上穿：前一日收盘 <= 前一日MA10 且 当日收盘 > 当日MA10
            boolean crossAbove = prevClose.compareTo(prevMa) <= 0 && currClose.compareTo(currMa) > 0;

            // 下穿：前一日收盘 >= 前一日MA10 且 当日收盘 < 当日MA10
            boolean crossBelow = prevClose.compareTo(prevMa) >= 0 && currClose.compareTo(currMa) < 0;

            if (crossAbove) {
                signals.add(buildSignal(0, code, curr, "收盘上穿10日均线"));
            } else if (crossBelow) {
                signals.add(buildSignal(1, code, curr, "收盘下穿10日均线"));
            }
        }
